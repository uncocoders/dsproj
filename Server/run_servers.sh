...
MACHINES=(lab2-28 lab2-29 lab2-30 localhost)
...
tmux new-session \; \
	...
	select-pane -t 0 \; \
	send-keys "ssh -t ${MACHINES[3]} \"cd $(pwd) > /dev/null; echo -n 'Connected to '; hostname; sleep .5s; cd ../Middleware ; ./run_middleware_test.sh ${MACHINES[0]} ${MACHINES[1]} ${MACHINES[2]}\"" C-m \;
